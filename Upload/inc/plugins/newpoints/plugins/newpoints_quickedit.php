...
{$footer}
    ...
    newpoints_add_template(
        ...
        '[<a href="{$mybb->settings[\'bburl\']}/newpoints.php?action=quickedit&amp;uid={$memprofile[\'uid\']}">{$lang->quickedit_edit_points}</a>]',
        '-1'
    );
    newpoints_add_template(
        'newpoints_quickedit_postbit',
        '[<a href="{$mybb->settings[\'bburl\']}/newpoints.php?action=quickedit&amp;uid={$post[\'uid\']}&amp;pid={$post[\'pid\']}">{$lang->quickedit_edit_points}</a>]',
        '-1'
    );
    newpoints_add_template(
        'newpoints_quickedit_shop',
        '<tr>
    ...
    newpoints_add_template(
        'newpoints_quickedit_shop_item',
        '<a href="{$mybb->settings[\'bburl\']}/newpoints.php?action=shop&amp;shop_action=view&amp;iid={$item[\'iid\']}"><img src="{$mybb->settings[\'bburl\']}/{$item[\'icon\']}" title="{$item[\'name\']}" style="vertical-align: middle;" /></a>
<label><input type="checkbox" class="checkbox" name="items[]" value="{$item[\'iid\']}" tabindex="{$tabindex}" /> {$item[\'name\']}</label>',
        '-1'
    );
    newpoints_add_template(
...
function newpoints_quickedit_deactivate()
{
    ...
    newpoints_remove_templates(
        "'newpoints_quickedit', 'newpoints_quickedit_profile', 'newpoints_quickedit_postbit', 'newpoints_quickedit_shop', 'newpoints_quickedit_shop_item', 'newpoints_quickedit_bank'"
    );
...
function newpoints_quickedit_install()
{
    ...
    newpoints_add_setting(
        'newpoints_quickedit_shop_on',
        'newpoints_shop',
        $lang->quickedit_shop_on_n,
        $lang->quickedit_shop_on_d,
        'yesno',
        0,
        -2
    );
    newpoints_add_setting(
        'newpoints_quickedit_shop_stock',
        'newpoints_shop',
        $lang->quickedit_shop_stock_n,
        $lang->quickedit_shop_stock_d,
        'yesno',
        0,
        -1
    );
    newpoints_add_setting(
        'newpoints_quickedit_bank_on',
        'newpoints_bank',
        $lang->quickedit_bank_on_n,
        $lang->quickedit_bank_on_d,
        'yesno',
        0,
        5
    );
...
function newpoints_quickedit_uninstall()
{
    ...
    newpoints_remove_log(['quickedit']);
}
...
function newpoints_quickedit_is_installed()
{
    ...
    $query = $db->simple_select('newpoints_settings', 'sid', 'name=\'newpoints_quickedit_on\'', ['limit' => 1]);
...
function newpoints_quickedit_start()
{
    ...
    if ($mybb->settings['newpoints_quickedit_on'] == 1 && $mybb->input['action'] == 'quickedit' && ($mybb->usergroup['cancp'] == 1 || $mybb->usergroup['cancp'] == 1)) {
        ...
        if ($mybb->request_method == 'post') {
            ...
            if (function_exists(
                    'newpoints_shop_page'
                ) && !empty($user['newpoints_items']) && $mybb->input['items'] && $mybb->settings['newpoints_quickedit_shop_on'] == 1) {
                ...
                sort($user_items);
                $db->update_query('users', ['newpoints_items' => serialize($user_items)], "uid='{$user['uid']}'");
            }
            //*\\ Newpoints Shop Code END //*\\
            //*\\ Newpoints Bank Code START //*\\
            if (function_exists(
                    'newpoints_bank_page'
                ) && $mybb->settings['newpoints_quickedit_bank_on'] == 1 && $mybb->input['newpoints_bankoffset'] != $user['newpoints_bankoffset']) {
                $db->update_query(
                    'users',
                    ['newpoints_bankoffset' => floatval($mybb->input['newpoints_bankoffset'])],
                    "uid='{$user['uid']}'"
                );
            }
            //*\\ Newpoints Bank Code END //*\\
            ...
            $lang->quickedit_log = $lang->sprintf(
                ...
                newpoints_format_points($user['newpoints'])
            );
        ...
        if (function_exists(
                'newpoints_shop_page'
            ) && !empty($user['newpoints_items']) && $mybb->settings['newpoints_quickedit_shop_on'] == 1) {
